...
app.controller('WorkspaceController', function($scope, CubeService) {
	...
	function zoomCubes() {
		...
		//console.log($scope.selectedCubes);
	...
	// need this for some reason to make sure only 
	// selected cubes show. Else some don't go away...
	$scope.$watch('dataView', function(newValue, oldValue) {
		if (newValue!='data_zoom') $scope.selectedCubes = {};
	});
